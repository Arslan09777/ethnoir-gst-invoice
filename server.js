...
async function shopifyOrders() {
  const shop = process.env.SHOPIFY_SHOP, token = process.env.SHOPIFY_ADMIN_ACCESS_TOKEN || oauthAccessToken;
...
http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/auth/callback') {
    ...
    try {
      const shop = process.env.SHOPIFY_SHOP;
      const response = await fetch(`https://${shop}/admin/oauth/access_tokens`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ client_id: process.env.SHOPIFY_API_KEY, client_secret: process.env.SHOPIFY_API_SECRET, code: params.get('code') }) });
  ...
  if (url.pathname === '/api/health') return json(res, 200, { configured: Boolean(process.env.SHOPIFY_SHOP && (process.env.SHOPIFY_ADMIN_ACCESS_TOKEN || oauthAccessToken)), shop: process.env.SHOPIFY_SHOP || null });
